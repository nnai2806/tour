...
const OutstandingHome = () => {
  ...
  const fetchTours = async (page = 0) => {
    const res = await axios.get(
      `/api/tour/get?page=${page}&isOutstanding=true`
    );
    return res.data;
  };
  ...
  return (
    <div className="mt-10">
      ...
      {/* List tours */}
      <div className="relative grid gap-4 md:grid-cols-3 sm:grid-cols-2">
        {data?.tours.map((item) => (
          <div
          ...
          >
            <div className="badge badge-lg top-0 left-0 badge-primary absolute">
              Nổi bật
            </div>
...
export default OutstandingHome;
